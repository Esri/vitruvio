...
TArray<UCityEngineComponent*> SReplacementDialogWidget::GetCityEngineActorsToApplyReplacements(bool bIncludeAll) const
{
	TArray<UCityEngineComponent*> ApplyToComponents;
	ApplyToComponents.Add(CityEngineComponent);

	if (bIncludeAll)
	{
		TArray<AActor*> Actors;
		if (const UWorld* World = GEngine->GetWorldFromContextObject(CityEngineComponent, EGetWorldErrorMode::LogAndReturnNull))
		{
			for (TActorIterator<AActor> It(World, AActor::StaticClass()); It; ++It)
			{
				if (UCityEngineComponent* Component = It->FindComponentByClass<UCityEngineComponent>())
				{
					if (Component->GetRpk() == CityEngineComponent->GetRpk())
					{
...
void SReplacementDialogWidget::Construct(const FArguments& InArgs)
{
	...
	CityEngineComponent = InArgs._VitruvioComponent;
