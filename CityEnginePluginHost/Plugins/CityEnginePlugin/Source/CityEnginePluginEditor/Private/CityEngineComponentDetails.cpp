...
void UpdateAttributeValue(UCityEngineComponent* CityEngineComponent, A* Attribute, const V& Value)
{
	Attribute->Value = Value;
	Attribute->bUserSet = true;
	CityEngineComponent->EvaluateRuleAttributes(CityEngineComponent->GenerateAutomatically);
}

bool IsVitruvioComponentSelected(const TArray<TWeakObjectPtr<UObject>>& ObjectsBeingCustomized, UCityEngineComponent*& OutComponent)
{
	...
	for (size_t ObjectIndex = 0; ObjectIndex < ObjectsBeingCustomized.Num(); ++ObjectIndex)
	{
		...
		{
			if (UCityEngineComponent* CityEngineComponent = Cast<UCityEngineComponent>(CurrentObject.Get()))
			{
				OutComponent = CityEngineComponent;
				return true;
			}
		}
	}
...
FResetToDefaultOverride ResetToDefaultOverride(URuleAttribute* Attribute, UCityEngineComponent* CityEngineComponent)
{
	FResetToDefaultOverride ResetToDefaultOverride = FResetToDefaultOverride::Create(
		...
		FResetToDefaultHandler::CreateLambda([Attribute, CityEngineComponent](TSharedPtr<IPropertyHandle> Property) {
...
void AddScalarWidget(const TArray<TSharedRef<IDetailTreeNode>> DetailTreeNodes, IDetailGroup& Group, URuleAttribute* Attribute,
					 UCityEngineComponent* CityEngineComponent)
{
	...
	ValueRow.OverrideResetToDefault(ResetToDefaultOverride(Attribute, CityEngineComponent));
					...
					UCityEngineComponent* CityEngineComponent)
{
	...
	if (ValuesArrayRoot)
	{
		...
		Row.OverrideResetToDefault(ResetToDefaultOverride(Attribute, CityEngineComponent));
...
void AddGenerateButton(IDetailCategoryBuilder& RootCategory, UCityEngineComponent* CityEngineComponent)
{
	// clang-format off
	RootCategory.AddCustomRow(FText::FromString(L"Generate"), false)
	.WholeRowContent()
	.VAlign(VAlign_Center)
	...
	[
		...
		[
			SNew(SButton)
			.OnClicked_Lambda([CityEngineComponent]()
			{
				CityEngineComponent->Generate();
				return FReply::Handled();
			}).Content()
			[
				SNew(STextBlock)
				.Text(FText::FromString(FString(TEXT("Generate"))))
				.Font(IDetailLayoutBuilder::GetDetailFont())
			]
		]
		.VAlign(VAlign_Fill)
...
void OpenReplacementDialog(UCityEngineComponent* CityEngineComponent, bool bNeedsRegenerate)
{
	...
	if (bNeedsRegenerate)
	{
		...
		Proxy->OnGenerateCompleted.AddLambda(
			[CityEngineComponent, OnDialogClosed]() { TInstanceDialogType::OpenDialog(CityEngineComponent, OnDialogClosed, true); });
		CityEngineComponent->Generate(Proxy, {true, true});
	}
	else
	{
		TInstanceDialogType::OpenDialog(CityEngineComponent, OnDialogClosed, false);
	}

	FCityEngineEditorModule::Get().BlockUntilGenerated();
}

void AddReplacementButtons(IDetailCategoryBuilder& RootCategory, UCityEngineComponent* CityEngineComponent)
{
	bool bHasReplacement = CityEngineComponent->InstanceReplacement != nullptr || CityEngineComponent->MaterialReplacement != nullptr;
	...
	[
		...
		[
			SNew(SButton)
			.OnClicked_Lambda([CityEngineComponent, bHasReplacement]()
			{
				OpenReplacementDialog<FMaterialReplacementDialog>(CityEngineComponent, bHasReplacement);
				return FReply::Handled();
			})
		...
		[
			...
			.OnClicked_Lambda([CityEngineComponent, bHasReplacement]()
			{
				OpenReplacementDialog<FInstanceReplacementDialog>(CityEngineComponent, bHasReplacement);
				return FReply::Handled();
			})
			.IsEnabled(TAttribute<bool>::CreateLambda([]()
			{
...
FCityEngineComponentDetails::FCityEngineComponentDetails()
{
	for (const auto& InitialShapeType : UCityEngineComponent::GetInitialShapesClasses())
	{
	...
	UCityEngineComponent::OnAttributesChanged.AddRaw(this, &FCityEngineComponentDetails::OnAttributesChanged);
	UCityEngineComponent::OnHierarchyChanged.AddRaw(this, &FCityEngineComponentDetails::OnVitruvioComponentHierarchyChanged);
}

FCityEngineComponentDetails::~FCityEngineComponentDetails()
{
	UCityEngineComponent::OnAttributesChanged.RemoveAll(this);
	UCityEngineComponent::OnHierarchyChanged.RemoveAll(this);
}

TSharedRef<IDetailCustomization> FCityEngineComponentDetails::MakeInstance()
{
	return MakeShareable(new FCityEngineComponentDetails);
}

void FCityEngineComponentDetails::BuildAttributeEditor(IDetailLayoutBuilder& DetailBuilder, IDetailCategoryBuilder& RootCategory,
													 UCityEngineComponent* CityEngineComponent)
{
	if (!CityEngineComponent || !CityEngineComponent->GetRpk())
	{
		return;
	}
	...
	IDetailGroup& RootGroup = RootCategory.AddGroup("Attributes", FText::FromString("Attributes"), false);
	...
	FResetToDefaultOverride ResetAllToDefaultOverride =
		FResetToDefaultOverride::Create(FResetToDefaultHandler::CreateLambda([CityEngineComponent](TSharedPtr<IPropertyHandle> Property) {
			for (const auto& AttributeEntry : CityEngineComponent->GetAttributes())
			{
	...
	for (const auto& AttributeEntry : CityEngineComponent->GetAttributes())
	{
		...
		Generator->OnFinishedChangingProperties().AddLambda([this, CityEngineComponent, Attribute](FPropertyChangedEvent Event) {
		...
		{
			AddArrayWidget(DetailTreeNodes, *Group, Attribute, CityEngineComponent);
		}
		else
		{
			AddScalarWidget(DetailTreeNodes, *Group, Attribute, CityEngineComponent);
		}
	}
}

void FCityEngineComponentDetails::AddSwitchInitialShapeCombobox(IDetailCategoryBuilder& RootCategory,
															  ...
															  UCityEngineComponent* CityEngineComponent)
{
	...
	[
		...
		[
			...
			.OnSelectionChanged_Lambda([this,CityEngineComponent](TSharedPtr<FString> Selection, ESelectInfo::Type SelectInfo)
			{
				if (Selection.IsValid())
				{
					...
					if (TempInitialShape->ShouldConvert(CityEngineComponent->InitialShape->GetPolygon()))
					{
						GEditor->BeginTransaction(*FGuid::NewGuid().ToString(), FText::FromString("Change Initial Shape Type"),CityEngineComponent->GetOwner());
						CityEngineComponent->Modify();
						CityEngineComponent->SetInitialShapeType(InitialShapeTypeMap[Selection]);
						CityEngineComponent->Generate();

						// Hack to refresh the property editor
						GEditor->SelectActor(CityEngineComponent->GetOwner(), false, true, true, true);
						GEditor->SelectActor(CityEngineComponent->GetOwner(), true, true, true, true);
						GEditor->SelectComponent(CityEngineComponent, true, true, true);
						GEditor->EndTransaction();
					}
					else
					{
						const TSharedPtr<FString>& CurrentSelection = InitialShapeClassMap[CityEngineComponent->InitialShape->GetClass()];
...
void FCityEngineComponentDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	TSharedPtr<IPropertyHandle> GenerateAutomaticallyProperty =
		DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UCityEngineComponent, GenerateAutomatically), UCityEngineComponent::StaticClass());
	GenerateAutomaticallyProperty->SetOnPropertyValueChanged(
		FSimpleDelegate::CreateSP(this, &FCityEngineComponentDetails::OnGenerateAutomaticallyChanged));
	...
	UCityEngineComponent* CityEngineComponent = nullptr;

	if (IsVitruvioComponentSelected(ObjectsBeingCustomized,CityEngineComponent))
	{
		DetailBuilder.GetProperty(FName(TEXT("Attributes")))->MarkHiddenByCustomization();

		if (!CityEngineComponent->InitialShape)
		{
			DetailBuilder.GetProperty(GET_MEMBER_NAME_CHECKED(UCityEngineComponent, InitialShape))->MarkHiddenByCustomization();
		}

		IDetailCategoryBuilder& CityEngineCategory = DetailBuilder.EditCategory("CityEngine");
		if (!CityEngineComponent->GenerateAutomatically)
		{
			AddGenerateButton(CityEngineCategory, CityEngineComponent);
		}

		if (!CityEngineComponent->IsBatchGenerated())
		{
			IDetailCategoryBuilder& ReplacementsCategory = DetailBuilder.EditCategory("CityEngine Replacements");
			AddReplacementButtons(CityEngineCategory, CityEngineComponent);
		}

		if (CityEngineComponent->InitialShape && CityEngineComponent->CanChangeInitialShapeType())
		{
			...
			if (CityEngineComponent->InitialShape)
			{
				...
				{
					UClass* IsClass = CityEngineComponent->InitialShape->GetClass();
			...
			AddSwitchInitialShapeCombobox(CityEngineCategory, CurrentInitialShapeType, CityEngineComponent);
		}

		IDetailCategoryBuilder& AttributeCategory = DetailBuilder.EditCategory("CityEngine Attributes");
		AttributeCategory.SetSortOrder(2000);
		BuildAttributeEditor(DetailBuilder, AttributeCategory, CityEngineComponent);
	}
}

void FCityEngineComponentDetails::CustomizeDetails(const TSharedPtr<IDetailLayoutBuilder>& DetailBuilder)
{
	CachedDetailBuilder = DetailBuilder;
	CustomizeDetails(*DetailBuilder);
}

void FCityEngineComponentDetails::OnGenerateAutomaticallyChanged()
{
...
void FCityEngineComponentDetails::OnVitruvioComponentHierarchyChanged(UCityEngineComponent* Component)
{
...
void FCityEngineComponentDetails::OnAttributesChanged(UObject* Object, struct FPropertyChangedEvent& Event)
{
	...
	if (PropertyName == FName(TEXT("Attributes")))
	{
		...
		{
			UObject* ObjectModified = Objects[0].Get();
			UCityEngineComponent* Component = Cast<UCityEngineComponent>(Object);
			AActor* Owner = nullptr;
			if (Component)
			{
				Owner = Component->GetOwner();
