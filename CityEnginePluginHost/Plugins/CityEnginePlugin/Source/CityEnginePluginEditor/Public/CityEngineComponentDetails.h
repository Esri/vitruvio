...
class FCityEngineComponentDetails final : public IDetailCustomization
{
public:
	FCityEngineComponentDetails();
	~FCityEngineComponentDetails();
	...
	void AddSwitchInitialShapeCombobox(IDetailCategoryBuilder& RootCategory, const TSharedPtr<FString>& CurrentInitialShapeType,
									   UCityEngineComponent* VitruvioComponent);
	...
	void OnVitruvioComponentHierarchyChanged(UCityEngineComponent* Component);

private:
	void BuildAttributeEditor(IDetailLayoutBuilder& DetailBuilder, IDetailCategoryBuilder& RootCategory, UCityEngineComponent* VitruvioActor);
