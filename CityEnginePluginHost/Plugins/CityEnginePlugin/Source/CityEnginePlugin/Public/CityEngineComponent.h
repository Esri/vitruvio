...
#include "CityEngineComponent.generated.h"

DECLARE_LOG_CATEGORY_EXTERN(LogCityEngineComponent, Log, All);
...
struct FGenerateOptions
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "CityEngine")
...
{
	FString Name;
	TSharedPtr<FCityEngineMesh> InstanceMesh;
...
{
	TSharedPtr<FCityEngineMesh> ShapeMesh;
...
UCLASS(ClassGroup = (Custom), meta = (BlueprintSpawnableComponent, DisplayName = "ArcGIS CityEngine Component"))
class CITYENGINEPLUGIN_API UCityEngineComponent : public UActorComponent
{
	GENERATED_BODY()
...
public:
	UCityEngineComponent();

	/** Automatically generate after changing attributes or properties. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, DisplayName = "Generate Automatically", Category = "CityEngine",
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, DisplayName = "Hide Initial Shape after Generation", Category = "CityEngine",
		meta = (EditCondition = "!bBatchGenerate", EditConditionHides))
	...
	UPROPERTY(VisibleAnywhere, Instanced, Category = "CityEngine")
	...
	/* Initialize the CityEngineComponent. Only needs to be called if the Component is natively attached. */
	void Initialize();

	/* Removes the generated meshes from this CityEngineComponent. */
	...
	UPROPERTY(BlueprintAssignable, Category = "CityEngine")
	...
	DECLARE_MULTICAST_DELEGATE_OneParam(FOnHierarchyChanged, UCityEngineComponent*);
...
private:
	...
	UPROPERTY(EditAnywhere, DisplayName = "Rule Package", Category = "CityEngine", meta = (AllowPrivateAccess = "true"))
	URulePackage* Rpk;

	/** Rule attributes used for generation. */
	UPROPERTY(EditAnywhere, DisplayName = "Attributes", Category = "CityEngine")
	TMap<FString, URuleAttribute*> Attributes;

	/** Random seed used for generation. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, DisplayName = "Random Seed", Category = "CityEngine", meta = (AllowPrivateAccess = "true"))
	int32 RandomSeed;

	/** CGA Reports from generation. */
	UPROPERTY(VisibleAnywhere, DisplayName = "Reports", Category = "CityEngine")
	TMap<FString, FReport> Reports;

	/** Generate several CityEngineComponents together in batches which can improve generate as well as rendering performance. */
	UPROPERTY(EditAnywhere, DisplayName = "Batch Generate", Category = "CityEngine")
	...
	// Note that these are only unique per CityEngineComponent
	UPROPERTY()
	TMap<UMaterialInterface*, FString> MaterialIdentifiers;
	TMap<FString, int32> UniqueMaterialIdentifiers;
	...
	friend class ACityEngineBatchActor;
};