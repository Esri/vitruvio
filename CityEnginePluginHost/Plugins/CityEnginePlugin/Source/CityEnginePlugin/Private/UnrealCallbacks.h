...
struct FModelDescription
{
	...
	TMap<CityEngine::FMaterialAttributeContainer, FPolygonGroupID> MaterialToPolygonMap;
};
...
public:
	...
	const CityEngine::FInstanceMap& GetInstances() const
	{
		return Instances;
	}

	const TMap<FString, TSharedPtr<FCityEngineMesh>>& GetInstanceMeshes() const
	{
		return InstanceMeshes;
	}

	const TSharedPtr<FCityEngineMesh>& GetGeneratedModel() const
	{
