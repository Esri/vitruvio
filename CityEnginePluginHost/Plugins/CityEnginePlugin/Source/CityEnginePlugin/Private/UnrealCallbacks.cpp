...
// clang-format off
const TMap<CityEngine::EPrtUvSetType, CityEngine::EUnrealUvSetType> PRTToUnrealUVSetMap = {
	{CityEngine::EPrtUvSetType::ColorMap,     CityEngine::EUnrealUvSetType::ColorMap},
	{CityEngine::EPrtUvSetType::OpacityMap,   CityEngine::EUnrealUvSetType::OpacityMap},
	{CityEngine::EPrtUvSetType::NormalMap,    CityEngine::EUnrealUvSetType::NormalMap},
	{CityEngine::EPrtUvSetType::EmissiveMap,  CityEngine::EUnrealUvSetType::EmissiveMap},
	{CityEngine::EPrtUvSetType::RoughnessMap, CityEngine::EUnrealUvSetType::RoughnessMap},
	{CityEngine::EPrtUvSetType::MetallicMap,  CityEngine::EUnrealUvSetType::MetallicMap},
	{CityEngine::EPrtUvSetType::DirtMap,      CityEngine::EUnrealUvSetType::DirtMap}};

const TMap<CityEngine::EUnrealUvSetType, FString> UnrealUVSetToMaterialParamStringMap = {
	{CityEngine::EUnrealUvSetType::DirtMap,      TEXT("HasDirtMapUV")},
	{CityEngine::EUnrealUvSetType::OpacityMap,   TEXT("HasOpacityMapUV")},
	{CityEngine::EUnrealUvSetType::NormalMap,    TEXT("HasNormalMapUV")},  
	{CityEngine::EUnrealUvSetType::EmissiveMap,  TEXT("HasEmissiveMapUV")},
	{CityEngine::EUnrealUvSetType::RoughnessMap, TEXT("HasRoughnessMapUV")}, 
	{CityEngine::EUnrealUvSetType::MetallicMap,  TEXT("HasMetallicMapUV")}
};
// clang-format on
...
TMap<FString, double> CreateAvailableUVSetMaterialParameterMap(uint32_t const* const* UVCounts, size_t UVSets)
{
	...
	{
		const CityEngine::EUnrealUvSetType* UnrealUVSetPtr = PRTToUnrealUVSetMap.Find(static_cast<CityEngine::EPrtUvSetType>(PrtUvSet));
		...
		if (bIsValidUnrealUVSet)
		{
			...
			const CityEngine::EUnrealUvSetType UnrealUVSet = *UnrealUVSetPtr;
			if (UnrealUVSet != CityEngine::EUnrealUvSetType::ColorMap && UnrealUVSet != CityEngine::EUnrealUvSetType::None)
			{
	...
	double const* const* uvs, uint32_t const* const* uvCounts, uint32_t const* const* uvIndices, size_t uvSets, const uint32_t* faceRanges, size_t faceRangesSize, const prt::AttributeMap** materials)
{
	...
	for (size_t PolygonGroupIndex = 0; PolygonGroupIndex < faceRangesSize; ++PolygonGroupIndex)
	{
		...
		CityEngine::FMaterialAttributeContainer MaterialContainer(materials[PolygonGroupIndex]);
		...
		if (ModelDescription.MaterialToPolygonMap.Contains(MaterialContainer))
		{
			PolygonGroupId = ModelDescription.MaterialToPolygonMap[MaterialContainer];
		}
		else
		{
			ModelDescription.Materials.Add(MaterialContainer);
			PolygonGroupId = ModelDescription.MeshDescription.CreatePolygonGroup();
			ModelDescription.MaterialToPolygonMap.Add(MaterialContainer, PolygonGroupId);
		}
		...
		for (size_t FaceIndex = 0; FaceIndex < PolygonFaceCount; ++FaceIndex)
		{
			...
			if (FaceVertexCount >= 3)
			{
				for (size_t FaceVertexIndex = 0; FaceVertexIndex < FaceVertexCount; ++FaceVertexIndex)
				{
					...
					FVertexInstanceID InstanceId = ModelDescription.MeshDescription.CreateVertexInstance(FVertexID(VertexIndex + ModelDescription.VertexIndexOffset));
				...
				ModelDescription.MeshDescription.CreatePolygon(PolygonGroupId, PolygonVertexInstances);
				PolygonFaces++;
...
TSharedPtr<FCityEngineMesh> CreateCityEngineMesh(const FString& Identifier, FMeshDescription Description, TArray<CityEngine::FMaterialAttributeContainer> ModelMaterials)
{
	...
	return MakeShared<FCityEngineMesh>(Identifier, Description, ModelMaterials);
}
                              ...
                              const uint32_t* faceRanges, size_t faceRangesSize, const prt::AttributeMap** materials)
{
	...
	else
	{
		...
		if (const TSharedPtr<FCityEngineMesh> Mesh = FCityEngineModule::Get().GetMeshCache().Get(IdentifierString))
		{
		...
		if (!InstanceModelDescription.MeshDescription.IsEmpty())
		{
			...
			TSharedPtr<FCityEngineMesh> Mesh = CreateCityEngineMesh(IdentifierString, InstanceModelDescription.MeshDescription, InstanceModelDescription.Materials);
			Mesh = FCityEngineModule::Get().GetMeshCache().InsertOrGet(IdentifierString, Mesh);
...
void UnrealCallbacks::finish()
{
	if (!ModelDescription.MeshDescription.IsEmpty())
	{
		GeneratedModel = CreateCityEngineMesh(TEXT("GeneratedMesh"), ModelDescription.MeshDescription, ModelDescription.Materials);
	}
}
                                  ...
                                  size_t numInstanceMaterials)
{
	...
	TArray<CityEngine::FMaterialAttributeContainer> MaterialOverrides;
	if (instanceMaterials)
	{
		for (size_t MatIndex = 0; MatIndex < numInstanceMaterials; ++MatIndex)
		{
			CityEngine::FMaterialAttributeContainer MaterialContainer(instanceMaterials[MatIndex]);
...
{
	AttributeMapBuilders[isIndex]->setBoolArray(key, values, size);
	return prt::STATUS_OK;
}
...
{
	AttributeMapBuilders[isIndex]->setFloatArray(key, values, size);
	return prt::STATUS_OK;
}
...
prt::Status UnrealCallbacks::attrStringArray(size_t isIndex, int32_t shapeID, const wchar_t* key, const wchar_t* const* values, size_t size,
											 size_t nRows)
{
	AttributeMapBuilders[isIndex]->setStringArray(key, values, size);
	return prt::STATUS_OK;
}