...
#define LOCTEXT_NAMESPACE "CityEngineModule"
...
{
	const FString BaseDir = FPaths::ConvertRelativePathToFull(IPluginManager::Get().FindPlugin("CityEnginePlugin")->GetBaseDir());
...
void FCityEngineModule::InitializePrt()
{
	...
	RpkFolder = FPaths::CreateTempFilename(*TempDir, TEXT("CityEnginePluginForUnreal_"), TEXT(""));
}

void FCityEngineModule::StartupModule()
{
	// During cooking we do not start CityEngine
...
void FCityEngineModule::ShutdownModule()
{
	if (!Initialized)
	{
		return;
	...
	UE_LOG(LogUnrealPrt, Display,
		   TEXT("Shutting down CityEngine. Waiting for ongoing generate calls (%d), RPK loading tasks (%d) and attribute loading tasks (%d)"),
...
CityEngine::FTextureData FCityEngineModule::DecodeTexture(UObject* Outer, const FString& Path, const FString& Key) const
{
	const prt::AttributeMap* TextureMetadataAttributeMap = prt::createTextureMetadata(*Path, PrtCache.get());
	CityEngine::FTextureMetadata TextureMetadata = CityEngine::ParseTextureMetadata(TextureMetadataAttributeMap);
	...
	return CityEngine::DecodeTexture(Outer, Key, Path, TextureMetadata, std::move(Buffer), BufferSize);
}

FBatchGenerateResult FCityEngineModule::BatchGenerateAsync(TArray<FInitialShape> InitialShapes) const
{
...
FGenerateResultDescription FCityEngineModule::BatchGenerate(TArray<FInitialShape> InitialShapes) const
{
...
FGenerateResult FCityEngineModule::GenerateAsync(FInitialShape InitialShape) const
{
...
FGenerateResultDescription FCityEngineModule::Generate(const FInitialShape& InitialShape) const
{
	CHECK_PRT_INITIALIZED()
...
FAttributeMapResult FCityEngineModule::EvaluateRuleAttributesAsync(FInitialShape InitialShape) const
{
...
void FCityEngineModule::EvictFromResolveMapCache(URulePackage* RulePackage)
{
...
void FCityEngineModule::RegisterMesh(UStaticMesh* StaticMesh)
{
	FScopeLock Lock(&RegisterMeshLock);
	RegisteredMeshes.Add(StaticMesh);
}

void FCityEngineModule::UnregisterMesh(UStaticMesh* StaticMesh)
{
	FScopeLock Lock(&RegisterMeshLock);
	RegisteredMeshes.Remove(StaticMesh);
}

void FCityEngineModule::NotifyGenerateCompleted() const
{
...
TFuture<ResolveMapSPtr> FCityEngineModule::LoadResolveMapAsync(URulePackage* const RulePackage) const
{
	TPromise<ResolveMapSPtr> Promise;
...
IMPLEMENT_MODULE(FCityEngineModule, CityEnginePlugin)
