...
FInitialShapePolygon CreateInitialPolygonFromStaticMesh(const UStaticMesh* StaticMesh)
{
	...
	FInitialShapePolygon InitialShapePolygon = CityEngine::GetPolygon(MeshVertices, MeshIndices);
...
UStaticMesh* CreateDefaultStaticMesh()
{
	...
	const FString PackageName = TEXT("/Game/CityEngine/") + InitialShapeName;
...
USceneComponent* UStaticMeshInitialShape::CreateInitialShapeComponent(UCityEngineComponent* Component)
{
...
USceneComponent* UStaticMeshInitialShape::CreateInitialShapeComponent(UCityEngineComponent* Component, UStaticMesh* NewStaticMesh)
{
	AActor* Owner = Component->GetOwner();
	if (!Owner)
	{
...
void UStaticMeshInitialShape::UpdatePolygon(UCityEngineComponent* Component)
{
#if WITH_EDITOR
	Modify();
#endif
...
void UStaticMeshInitialShape::UpdateSceneComponent(UCityEngineComponent* Component)
{
	AActor* Owner = Component->GetOwner();
	if (!Owner)
	{
...
void UStaticMeshInitialShape::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
		...
		PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UStaticMeshInitialShape, InitialShapeMesh))
	{
		...
		{
			if (UCityEngineComponent* VitruvioComponent = Owner->FindComponentByClass<UCityEngineComponent>())
			{
...
USceneComponent* USplineInitialShape::CreateInitialShapeComponent(UCityEngineComponent* Component)
{
	AActor* Owner = Component->GetOwner();
	if (!Owner)
	{
...
USceneComponent* USplineInitialShape::CreateInitialShapeComponent(UCityEngineComponent* Component, const TArray<FSplinePoint>& SplinePoints)
{
	AActor* Owner = Component->GetOwner();
	if (!Owner)
	{
...
void USplineInitialShape::UpdatePolygon(UCityEngineComponent* Component)
{
#if WITH_EDITOR
	Modify();
#endif
...
void USplineInitialShape::UpdateSceneComponent(UCityEngineComponent* Component)
{
	AActor* Owner = Component->GetOwner();
	if (!Owner)
	{
