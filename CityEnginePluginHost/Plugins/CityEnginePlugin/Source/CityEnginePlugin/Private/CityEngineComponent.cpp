...
DEFINE_LOG_CATEGORY(LogCityEngineComponent);
...
template <typename T>
void SetInitialShape(UCityEngineComponent* Component, bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy, T&& InitialShapeInitializer)
{
...
template <typename A, typename T>
void SetAttribute(UCityEngineComponent* CityEngineComponent, const TMap<FString, URuleAttribute*>& Attributes, const FString& Name, const T& Value,
				  bool bEvaluateAttributes, bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy)
{
	...
	{
		CityEngineComponent->EvaluateRuleAttributes(bGenerateModel, CallbackProxy);
	}
}

template <typename A, typename T>
void EvaluateAndSetAttribute(UCityEngineComponent* CityEngineComponent, TMap<FString, URuleAttribute*>& Attributes, const FString& Name, const T& Value,
							 bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy)
{
	CityEngineComponent->Initialize();

	if (!CityEngineComponent->GetAttributesReady())
	{
		UGenerateCompletedCallbackProxy* Proxy = NewObject<UGenerateCompletedCallbackProxy>();
		Proxy->OnAttributesEvaluated.AddLambda(
			[=, &Attributes]() { SetAttribute<A, T>(CityEngineComponent, Attributes, Name, Value, true, bGenerateModel, CallbackProxy); });
		Proxy->RegisterWithGameInstance(CityEngineComponent);
		CityEngineComponent->EvaluateRuleAttributes(bGenerateModel, Proxy);
	}
	else
	{
		SetAttribute<A, T>(CityEngineComponent, Attributes, Name, Value, true, bGenerateModel, CallbackProxy);
	}
}

void EvaluateAndSetAttributes(UCityEngineComponent* CityEngineComponent, const TMap<FString, FString>& NewAttributes, bool bGenerateModel,
							  UGenerateCompletedCallbackProxy* CallbackProxy)
{
	...
	{
		UGenerateCompletedCallbackProxy* Proxy = NewObject<UGenerateCompletedCallbackProxy>();
		Proxy->OnAttributesEvaluated.AddLambda([=]() { EvaluateAndSetAttributes(CityEngineComponent, NewAttributes, bGenerateModel, CallbackProxy); });
		Proxy->RegisterWithGameInstance(CityEngineComponent);
		CityEngineComponent->EvaluateRuleAttributes(bGenerateModel, Proxy);
	}
	else
	{
		for (const auto& KeyValues : NewAttributes)
		{
			...
			URuleAttribute* const* AttributeResult = CityEngineComponent->GetAttributes().Find(Key);
			if (!AttributeResult)
			{
				continue;
			}
			...
			{
				SetAttribute<UFloatAttribute, double>(CityEngineComponent, CityEngineComponent->GetAttributes(), Key, FCString::Atof(*Value), false,
													  false, nullptr);
			}
			else if (Cast<UBoolAttribute>(Attribute))
			{
				SetAttribute<UBoolAttribute, bool>(CityEngineComponent, CityEngineComponent->GetAttributes(), Key, ToBool(Value), false, false, nullptr);
			}
			else if (Cast<UStringAttribute>(Attribute))
			{
				SetAttribute<UStringAttribute, FString>(CityEngineComponent, CityEngineComponent->GetAttributes(), Key, Value, false, false, nullptr);
			}
			else if (Cast<UArrayAttribute>(Attribute))
			{
				...
				if (Cast<UFloatArrayAttribute>(Attribute))
				{
					...
					SetAttribute<UFloatArrayAttribute, TArray<double>>(CityEngineComponent, CityEngineComponent->GetAttributes(), Key, DoubleValues,
																	   false, false, nullptr);
				}
				else if (Cast<UBoolArrayAttribute>(Attribute))
				{
					...
					SetAttribute<UBoolArrayAttribute, TArray<bool>>(CityEngineComponent, CityEngineComponent->GetAttributes(), Key, BoolValues, false,
																	false, nullptr);
				}
				else
				{
					SetAttribute<UStringArrayAttribute, TArray<FString>>(CityEngineComponent, CityEngineComponent->GetAttributes(), Key, StringValues,
																		 false, false, nullptr);
				}
			}
		}
...
#if WITH_EDITOR
bool IsRelevantObject(UCityEngineComponent* CityEngineComponent, UObject* Object)
{
	if (CityEngineComponent == Object || CityEngineComponent->InitialShape == Object)
	{
		return true;
	}
	AActor* Owner = CityEngineComponent->GetOwner();
	if (!Owner)
	{
		return false;
	}
...
UCityEngineComponent::FOnHierarchyChanged UCityEngineComponent::OnHierarchyChanged;
UCityEngineComponent::FOnAttributesChanged UCityEngineComponent::OnAttributesChanged;
									 ...
									 UWorld* World)
{
	...
	{
		const TSharedPtr<FCityEngineMesh>& VitruvioMesh = GenerateResult.InstanceMeshes[Key.MeshId];
		...
		{
			const CityEngine::FMaterialAttributeContainer& MaterialContainer = Key.MaterialOverrides[MaterialIndex];
...
UCityEngineComponent::UCityEngineComponent()
{
...
void UCityEngineComponent::CalculateRandomSeed()
{
...
bool UCityEngineComponent::HasValidInputData() const
{
	return InitialShape && InitialShape->IsValid() && Rpk;
}

bool UCityEngineComponent::IsReadyToGenerate() const
{
	return (HasValidInputData() && bAttributesReady) || bBatchGenerate;
}

void UCityEngineComponent::SetRpk(URulePackage* RulePackage, bool bEvaluateAttributes, bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy)
{
...
void UCityEngineComponent::SetBatchGenerated(bool bBatchGeneration)
{
	if (GenerateToken)
	{
	...
	UCityEngineBatchSubsystem* VitruvioSubsystem = GetWorld()->GetSubsystem<UCityEngineBatchSubsystem>();
	if (bBatchGenerate)
	{
		RemoveGeneratedMeshes();
		VitruvioSubsystem->RegisterCityEngineComponent(this);
	}
	else
	{
		VitruvioSubsystem->UnregisterCityEngineComponent(this);
		EvaluateRuleAttributes(true, nullptr);
	}
}

bool UCityEngineComponent::IsBatchGenerated() const
{
	return bBatchGenerate;
}

void UCityEngineComponent::SetMaterialReplacementAsset(UMaterialReplacementAsset* MaterialReplacementAsset)
{
	MaterialReplacement = MaterialReplacementAsset;
	Generate();
}

void UCityEngineComponent::SetInstanceReplacementAsset(UInstanceReplacementAsset* InstanceReplacementAsset)
{
	InstanceReplacement = InstanceReplacementAsset;
	Generate();
}

void UCityEngineComponent::SetStringAttribute(const FString& Name, const FString& Value, bool bGenerateModel,
											UGenerateCompletedCallbackProxy* CallbackProxy)
{
	EvaluateAndSetAttribute<UStringAttribute, FString>(this, this->Attributes, Name, Value, bGenerateModel, CallbackProxy);
}

bool UCityEngineComponent::GetStringAttribute(const FString& Name, FString& OutValue) const
{
	return GetAttribute<UStringAttribute, FString>(this->Attributes, Name, OutValue);
}

void UCityEngineComponent::SetStringArrayAttribute(const FString& Name, const TArray<FString>& Values, bool bGenerateModel,
												 UGenerateCompletedCallbackProxy* CallbackProxy)
{
...
bool UCityEngineComponent::GetStringArrayAttribute(const FString& Name, TArray<FString>& OutValue) const
{
	return GetAttribute<UStringArrayAttribute, TArray<FString>>(this->Attributes, Name, OutValue);
}

void UCityEngineComponent::SetBoolAttribute(const FString& Name, bool Value, bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy)
{
	EvaluateAndSetAttribute<UBoolAttribute, bool>(this, this->Attributes, Name, Value, bGenerateModel, CallbackProxy);
}

bool UCityEngineComponent::GetBoolAttribute(const FString& Name, bool& OutValue) const
{
	return GetAttribute<UBoolAttribute, bool>(this->Attributes, Name, OutValue);
}

void UCityEngineComponent::SetBoolArrayAttribute(const FString& Name, const TArray<bool>& Values, bool bGenerateModel,
											   UGenerateCompletedCallbackProxy* CallbackProxy)
{
	EvaluateAndSetAttribute<UBoolArrayAttribute, TArray<bool>>(this, this->Attributes, Name, Values, bGenerateModel, CallbackProxy);
}

bool UCityEngineComponent::GetBoolArrayAttribute(const FString& Name, TArray<bool>& OutValue) const
{
	return GetAttribute<UBoolArrayAttribute, TArray<bool>>(this->Attributes, Name, OutValue);
}

void UCityEngineComponent::SetFloatAttribute(const FString& Name, double Value, bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy)
{
	EvaluateAndSetAttribute<UFloatAttribute, double>(this, this->Attributes, Name, Value, bGenerateModel, CallbackProxy);
}

bool UCityEngineComponent::GetFloatAttribute(const FString& Name, double& OutValue) const
{
	return GetAttribute<UFloatAttribute, double>(this->Attributes, Name, OutValue);
}

void UCityEngineComponent::SetFloatArrayAttribute(const FString& Name, const TArray<double>& Values, bool bGenerateModel,
												UGenerateCompletedCallbackProxy* CallbackProxy)
{
	EvaluateAndSetAttribute<UFloatArrayAttribute, TArray<double>>(this, this->Attributes, Name, Values, bGenerateModel, CallbackProxy);
}

bool UCityEngineComponent::GetFloatArrayAttribute(const FString& Name, TArray<double>& OutValue) const
{
	return GetAttribute<UFloatArrayAttribute, TArray<double>>(this->Attributes, Name, OutValue);
}

void UCityEngineComponent::SetAttributes(const TMap<FString, FString>& NewAttributes, bool bGenerateModel,
									   UGenerateCompletedCallbackProxy* CallbackProxy)
{
	EvaluateAndSetAttributes(this, NewAttributes, bGenerateModel, CallbackProxy);
}

void UCityEngineComponent::SetMeshInitialShape(UStaticMesh* StaticMesh, bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy)
{
...
void UCityEngineComponent::SetSplineInitialShape(const TArray<FSplinePoint>& SplinePoints, bool bGenerateModel,
											   UGenerateCompletedCallbackProxy* CallbackProxy)
{
...
const TMap<FString, URuleAttribute*>& UCityEngineComponent::GetAttributes() const
{
	return Attributes;
}

URulePackage* UCityEngineComponent::GetRpk() const
{
	return Rpk;
}

const TMap<FString, FReport>& UCityEngineComponent::GetReports() const
{
	return Reports;
}

void UCityEngineComponent::SetInitialShapeVisible(bool bVisible)
{
	InitialShapeSceneComponent->SetVisibility(bVisible, false);
	InitialShapeSceneComponent->SetHiddenInGame(!bVisible);
}

void UCityEngineComponent::SetRandomSeed(int32 NewRandomSeed, bool bGenerateModel, UGenerateCompletedCallbackProxy* CallbackProxy)
{
...
int32 UCityEngineComponent::GetRandomSeed()
{
	return RandomSeed;
}

void UCityEngineComponent::LoadInitialShape()
{
	if (InitialShape)
	{
...
void UCityEngineComponent::Initialize()
{
	if (bInitialized)
	{
		return;
	...
	{
		PropertyChangeDelegate = FCoreUObjectDelegates::OnObjectPropertyChanged.AddUObject(this, &UCityEngineComponent::OnPropertyChanged);
	}
#endif
	...
	if (bBatchGenerate)
	{
		UCityEngineBatchSubsystem* BatchGenerateSubsystem = GetWorld()->GetSubsystem<UCityEngineBatchSubsystem>();
		BatchGenerateSubsystem->RegisterCityEngineComponent(this);
	}
	else
	{
...
void UCityEngineComponent::ProcessGenerateQueue()
{
...
void UCityEngineComponent::ProcessAttributesEvaluationQueue()
{
...
void UCityEngineComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	if (!bInitialized)
	{
		Initialize();
...
void UCityEngineComponent::NotifyAttributesChanged()
{
#if WITH_EDITOR
	// Notify possible listeners (eg. Details panel) about changes to the Attributes
	FPropertyChangedEvent PropertyEvent(GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UCityEngineComponent, Attributes)));
	OnAttributesChanged.Broadcast(this, PropertyEvent);
#endif
}

void UCityEngineComponent::RemoveGeneratedMeshes()
{
...
bool UCityEngineComponent::GetAttributesReady() const
{
	return bAttributesReady;
}

bool UCityEngineComponent::HasGeneratedModel() const
{
	return bHasGeneratedModel;
}

UGeneratedModelStaticMeshComponent* UCityEngineComponent::GetGeneratedModelComponent() const
{
...
TArray<UGeneratedModelHISMComponent*> UCityEngineComponent::GetGeneratedModelHISMComponents() const
{
	if (!InitialShapeSceneComponent)
	{
		return {};
...
FString UCityEngineComponent::GetMaterialIdentifier(const UMaterialInterface* SourceMaterial) const
{
...
void UCityEngineComponent::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	if (GenerateToken)
	{
		GenerateToken->Invalidate();
...
void UCityEngineComponent::Generate(UGenerateCompletedCallbackProxy* CallbackProxy, const FGenerateOptions& GenerateOptions)
{
	Initialize();
	...
	if (InitialShape)
	{
		FGenerateResult GenerateResult =
			FCityEngineModule::Get().GenerateAsync({ FVector::ZeroVector, InitialShape->GetPolygon(), CityEngine::CreateAttributeMap(Attributes), RandomSeed, Rpk});
...
void UCityEngineComponent::PreEditUndo()
{
	Super::PreEditUndo();
...
void UCityEngineComponent::PostUndoRedo()
{
	DestroyInitialShapeComponent();
...
void UCityEngineComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UCityEngineComponent::OnPropertyChanged(UObject* Object, FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UCityEngineComponent, bBatchGenerate))
		{
		...
		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UCityEngineComponent, Rpk))
		{
			Attributes.Empty();
			bAttributesReady = false;
			bComponentPropertyChanged = true;
		...
		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UCityEngineComponent, RandomSeed))
		{
			bValidRandomSeed = true;
			bComponentPropertyChanged = true;
		}

		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UCityEngineComponent, HideAfterGeneration))
		{
			SetInitialShapeVisible(!(HideAfterGeneration && bHasGeneratedModel));
		}

		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UCityEngineComponent, GenerateAutomatically))
		{
			bComponentPropertyChanged = true;
		}
		...
		if (PropertyChangedEvent.Property->GetFName() == GET_MEMBER_NAME_CHECKED(UCityEngineComponent, InstanceReplacement))
		{
			bComponentPropertyChanged = true;
		}
	}
...
void UCityEngineComponent::SetInitialShapeType(const TSubclassOf<UInitialShape>& Type)
{
	RemoveGeneratedMeshes();
...
void UCityEngineComponent::EvaluateRuleAttributes(bool ForceRegenerate, UGenerateCompletedCallbackProxy* CallbackProxy)
{
	Initialize();
	...
	FAttributeMapResult AttributesResult =
		FCityEngineModule::Get().EvaluateRuleAttributesAsync({ FVector::ZeroVector, InitialShape->GetPolygon(), CityEngine::CreateAttributeMap(Attributes), RandomSeed, Rpk});
...
void UCityEngineComponent::InitializeInitialShapeComponent()
{
	if (InitialShapeSceneComponent)
	{
		return;
...
void UCityEngineComponent::DestroyInitialShapeComponent()
{
	if (!InitialShapeSceneComponent)
	{
		return;
...
TArray<TSubclassOf<UInitialShape>> UCityEngineComponent::GetInitialShapesClasses()
{
