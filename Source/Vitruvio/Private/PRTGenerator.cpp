 ...
 */
FGenerator::FGenerator(APRTActor* InPRTActor, FVitruvioModule* InVitruvioModule)
{
	...
	VitruvioModule = InVitruvioModule;
...
uint32 FGenerator::Run()
{
	...
	while (StopTaskCounter.GetValue() == 0) // && !IsFinished())
	{
		...
		if(bPRTGenerating && !bPRTDataReady)
		{
			...
			const auto Status = VitruvioModule->GenerateModel();
