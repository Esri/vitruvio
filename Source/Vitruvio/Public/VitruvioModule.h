...
struct FVitruvioModuleArgument
{
	FString KeyName;
	int32 Type;
	bool bValue;
...
struct FPRTAttribute
{
	...
	float fValue;
	FString sValue;
	TArray<FVitruvioModuleArgument> Arguments;
};
...
public:
	 ...
	 */
	static inline IVitruvioModule& Get()
	{
		return FModuleManager::LoadModuleChecked< IVitruvioModule >("VitruvioModule");
	}
	...
	{
		return FModuleManager::Get().IsModuleLoaded("VitruvioModule");
	}
};


class FVitruvioModule : public IVitruvioModule
{
	friend APRTActor;
