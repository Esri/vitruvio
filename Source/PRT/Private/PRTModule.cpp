...
#define LOCTEXT_NAMESPACE "FPRTModule"

IMPLEMENT_MODULE(FPRTModule, PRT);

TMap<FString, FPRTAttribute> FPRTModule::Attributes;
prt::Status FPRTModule::PluginStatus;

#define LOG_VERBOSE true

void FPRTModule::StartupModule()
{
 ...
 * \brief 
 */
void FPRTModule::InitializeSlateAttributePanel()
{
...
void FPRTModule::ShutdownModule()
{
 ...
 * \return 
 */
auto FPRTModule::GenerateAttributeResult() -> prt::Status
{
...
auto FPRTModule::GenerateModel(TMap<FString, FPRTAttribute> InAttributes) -> prt::Status
{
	ApplyAttributesToProceduralRuntime(InAttributes);
	return GenerateModel();
}

auto FPRTModule::GenerateModel() -> prt::Status
{
	bIsGenerating = true;
	bIsCompleted = false;

#if LOG_VERBOSE
	PRTLog.Message(L"FPRTModule::GenerateModel Called");
#endif
	...
	if (GenerateStatus != prt::STATUS_OK)
	{
		PRTLog.Message(TEXT(">> FPRTModule::GenerateModel() has encountered a problem: "),
		       prt::getStatusDescription(GenerateStatus), ELogVerbosity::Warning);
		return GenerateStatus;
	}
...
auto FPRTModule::LoadRPKDataToMemory() -> prt::Status
{
	size_t Size;
...
void FPRTModule::CreateVertexDataFaceLine(FVertData& OutData, const FString Line, const FString Material)
{
...
auto FPRTModule::CreateVertexDataFace(FVertData& OutData, const FString Value, const FString Material) const -> prt::Status
{
	int32 v = 0, u = 0, n = 0, i;
...
void FPRTModule::EmptyVertexData()
{
...
void FPRTModule::CreateVertexData()
{
	EmptyVertexData();
...
auto FPRTModule::CreateMaterialData() -> prt::Status
{
#if LOG_VERBOSE
	PRTLog.Message(TEXT("FPRTModule::CreateMaterialData"));
#endif
 ...
 * \return prt::Status
 */
auto FPRTModule::SetRPKFile(FString InRPKFile) -> prt::Status
{
	RPKStatus = prt::STATUS_RESOLVEMAP_PROVIDER_NOT_FOUND; //set it to this until everything gets loaded.
	
#if LOG_VERBOSE
	PRTLog.Message(TEXT("FPRTModule::SetRPKFile"));
#endif
	...
	if(PluginStatus != prt::STATUS_OK)
	{
		PRTLog.Message(TEXT("FPRTModule::InitializeRPK() Plugin Status error. Status: "), PluginStatus, ELogVerbosity::Warning);
		return PluginStatus;
	}
...
auto FPRTModule::SetRule(FString InRuleFile) -> prt::Status
{
	if (RPKStatus != prt::STATUS_OK)
	{
		PRTLog.Message(TEXT("FPRTModule::SetRule - PKStatus error: "), RPKStatus, ELogVerbosity::Warning);
		return RPKStatus;
	}
	...
	PRTLog.Message(TEXT("FPRTModule::SetRule: File: "), *RuleFile);
	...
	{
		PRTLog.Message(TEXT(">> FPRTModule::SetRule: There was a problem setting the rule file. "), ELogVerbosity::Warning);
		RPKStatus = prt::STATUS_NO_RULEFILE;
	}
	return RPKStatus;
}

void FPRTModule::ApplyAttributesToProceduralRuntime(TMap<FString, FPRTAttribute> InAttributes)
{
	PRTUtil.SetCurrentWorkingDirectoryToPlugin();
#if LOG_VERBOSE
	PRTLog.Message(TEXT("FPRTModule::ApplyAttributesToProceduralRuntime"));
#endif
...
void FPRTModule::SetInitialShape(FString InObjFile)
{
	FString FullPath;
#if LOG_VERBOSE
	//PRTLog.Message(TEXT("FPRTModule::SetInitialShape: File: "), InObjFile);
#endif 

	if (InObjFile.Len() < 1)
	{
		PRTLog.Message(TEXT("FPRTModule::SetInitialShape: Using Default Square.obj"));
...
#if LOG_VERBOSE
	PRTLog.Message(L"FPRTModule::SetInitialShape, File: %s", OBJFile);
#endif 
}

auto FPRTModule::CreateInitialShape() -> prt::Status
{
	if(RPKStatus != prt::STATUS_OK)
	{
		PRTLog.Message(TEXT("FPRTModule::CreateInitialShape RPK Status error. Status: "), RPKStatus, ELogVerbosity::Warning);
		return RPKStatus;
	}

	if (StartRule == nullptr)
	{
		PRTLog.Message(TEXT("FPRTModule::CreateInitialShape. Start Rule == nullptr."), ELogVerbosity::Warning);
	...
	if (AttributeMap == nullptr)
	{
		//PRTLog.Message(TEXT("FPRTModule::CreateInitialShape: Creating Attribute Map."));
		prt::AttributeMapBuilder* Builder = prt::AttributeMapBuilder::create();
		AttributeMap = Builder->createAttributeMapAndReset();
	}
	...
	{
		PRTLog.Message(TEXT(">> FPRTModule::CreateInitialShape error. Status: "), GeometryStatus, ELogVerbosity::Warning);
	}

	InitialShape = InitialShapeBuilder->createInitialShapeAndReset();
	
#if LOG_VERBOSE
	PRTLog.Message(TEXT("FPRTModule::CreateInitialShape. Status: "), GeometryStatus);
#endif
	
	return GeometryStatus;
}

TArray<FString> FPRTModule::GetRules()
{
#if LOG_VERBOSE
	PRTLog.Message(TEXT("FPRTModule::GetRules"));
#endif
	...
	{
		PRTLog.Message(TEXT(">> FPRTModule::GetRules. PRKStatus: "), RPKStatus, ELogVerbosity::Warning);
	}
	return Rules;
}

TMap<FString, FPRTAttribute> FPRTModule::GetAttributes()
{
#if LOG_VERBOSE
	PRTLog.Message(TEXT("FPRTModule::GetAttributes"));
#endif
			...
			for (size_t j = 0; j < Attribute->getNumAnnotations(); ++j)
			{
				...
				if (NumberOfArguments > 0)
				{
					for (size_t k = 0; k < NumberOfArguments; ++k)
					{
						...
						FPRTModuleArgument PreviousArgument;
	...
	{
		PRTLog.Message(TEXT(">> RPK Status error in FPRTModule::GetAttributes. Status: "), RPKStatus, ELogVerbosity::Warning);
	}
	return Attributes;
}
...
{
	if (!FPRTModule::Attributes.Contains(Key)) FPRTModule::Attributes.Add(Key);
	FPRTModule::Attributes[Key].KeyName = Key;
	FPRTModule::Attributes[Key].bValue = Value;
	FPRTModule::Attributes[Key].Type = prt::AAT_BOOL;
	return prt::STATUS_OK;
}
...
auto IAttributeResult::attrFloat(size_t IsIndex, int32_t ShapeID, const wchar_t* Key, double Value) -> prt::Status
{
	if (!FPRTModule::Attributes.Contains(Key)) FPRTModule::Attributes.Add(Key);
	FPRTModule::Attributes[Key].KeyName = Key;
	FPRTModule::Attributes[Key].fValue = Value;
	FPRTModule::Attributes[Key].Type = prt::AAT_FLOAT;
	return prt::STATUS_OK;
}
                                  ...
                                  const wchar_t* Value) -> prt::Status
{
	if (!FPRTModule::Attributes.Contains(Key)) FPRTModule::Attributes.Add(Key);
	FPRTModule::Attributes[Key].KeyName = Key;
	FPRTModule::Attributes[Key].sValue = Value;
	FPRTModule::Attributes[Key].Type = prt::AAT_STR;
	return prt::STATUS_OK;
}
...
bool FPRTModule::IsLoaded()
{
	if (PluginStatus != prt::STATUS_OK)
	{
		PRTLog.Message(TEXT("FPRTModule::IsLoaded() Plugin Status = "), PluginStatus, ELogVerbosity::Warning);

		if (RPKStatus != prt::STATUS_OK)
		{
			PRTLog.Message(TEXT("FPRTModule::IsLoaded() RPK Plugin Status = "), RPKStatus, ELogVerbosity::Warning);
		}
		return false;
	}
	return true;
...
bool FPRTModule::IsGenerating()
{
	return bIsGenerating;
}


bool FPRTModule::IsDone()
{
	return bIsCompleted;
}
